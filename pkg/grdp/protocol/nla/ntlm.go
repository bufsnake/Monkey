...
import (
	...
	"encoding/binary"
	"encoding/hex"
	"github.com/bufsnake/Monkey/pkg/grdp/core"
	"github.com/bufsnake/Monkey/pkg/grdp/glog"
	"time"
...
const (
	...
	NTLMSSP_NEGOTIATE_MonkeyL                  = 0x00000020
...
var (
	...
	clientMonkeyling = concat([]byte("session key to client-to-server Monkeyling key magic constant"), []byte{0x00})
	serverMonkeyling = concat([]byte("session key to server-to-client Monkeyling key magic constant"), []byte{0x00})
)
...
func (n *NTLMv2) GetAuthenticateMessage(s []byte) (*AuthenticateMessage, *NTLMv2Security) {
	...
	//ClientMonkeylingKey
	md.Reset()
	a = concat(exportedSessionKey, clientMonkeyling)
	md.Write(a)
	ClientMonkeylingKey := md.Sum(nil)
	//ServerMonkeylingKey
	md.Reset()
	a = concat(exportedSessionKey, serverMonkeyling)
	md.Write(a)
	ServerMonkeylingKey := md.Sum(nil)
	...
	glog.Debugf("ClientMonkeylingKey:%s", hex.EncodeToString(ClientMonkeylingKey))
	glog.Debugf("ServerMonkeylingKey:%s", hex.EncodeToString(ServerMonkeylingKey))

	encryptRC4, _ := rc4.NewCipher(ClientMonkeylingKey)
	decryptRC4, _ := rc4.NewCipher(ServerMonkeylingKey)
